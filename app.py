...
from ui.admin_dashboard_frame import AdminDashboardFrame # <-- DIIMPOR
...
class App(customtkinter.CTk):
    ...
    def create_main_layout(self):
        ...
        try:
            logo_path = os.path.join(db.get_base_path(), "assets", "BRINV.png")
            logo_img = customtkinter.CTkImage(Image.open(logo_path), size=(130, 70))
        ...
        # --- PERUBAHAN DI SINI: Membuat instance semua halaman, termasuk Admin Dashboard ---
        self.dashboard_frame = DashboardFrame(self.main_frame, explore_callback=lambda: self.select_frame_by_name("items"))
        self.admin_dashboard_frame = AdminDashboardFrame(self.main_frame, app=self) # <-- DITAMBAHKAN
    ...
    def select_frame_by_name(self, name):
        ...
        # Sembunyikan semua frame utama
        self.dashboard_frame.grid_forget()
        self.admin_dashboard_frame.grid_forget() # <-- DITAMBAHKAN
        self.item_frame.grid_forget()
        self.management_frame.grid_forget()
        self.history_frame.grid_forget()

        # --- PERUBAHAN LOGIKA DI SINI: Tampilkan dashboard sesuai status login ---
        if name == "dashboard":
            self.dashboard_button.configure(fg_color=self.active_button_color)
            if self.is_admin:
                self.admin_dashboard_frame.grid(row=0, column=0, sticky="nsew")
                self.admin_dashboard_frame.refresh_data() # Refresh data saat ditampilkan
            else:
    ...
    def successful_login(self):
        ...
        self.is_admin = True
        self.update_ui_for_login_status()
        self.select_frame_by_name("dashboard") # <-- Ini akan otomatis menampilkan Admin Dashboard
    ...
    def logout(self):
        ...
        if messagebox.askyesno("Konfirmasi Logout", "Apakah Anda yakin ingin logout?"):
            self.is_admin = False
            self.update_ui_for_login_status()
            self.select_frame_by_name("dashboard") # <-- Ini akan otomatis menampilkan Guest Dashboard
