...
class App(customtkinter.CTk):
    ...
    def create_main_layout(self):
        ...
        try:
            # --- PERUBAHAN DI SINI ---
            logo_path = os.path.join(db.get_base_path(), "assets", "BRINV.png") # <-- Ganti nama file
            
            # Sesuaikan ukuran (lebar, tinggi) agar logo terlihat bagus
            logo_img = customtkinter.CTkImage(Image.open(logo_path), size=(130, 70)) # <-- Sesuaikan ukuran
            
            customtkinter.CTkLabel(logo_frame, image=logo_img, text="").pack()
        except Exception:
            # Fallback ini akan muncul jika 'BRINV.png' tidak ditemukan
