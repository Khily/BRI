...
def get_total_items_count():
    """Mengembalikan jumlah total semua barang dalam inventaris."""
    conn = None
    try:
        conn = create_connection()
        if not (conn and conn.is_connected()): return 0
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(item_id) FROM items")
        count = cursor.fetchone()[0]
        return count if count is not None else 0
    except Error as e:
        logging.error(f"Error getting total items count: {e}")
        return 0
    finally:
        if conn and conn.is_connected(): cursor.close(); conn.close()

def get_total_categories_count():
    """Mengembalikan jumlah total kategori."""
    conn = None
    try:
        conn = create_connection()
        if not (conn and conn.is_connected()): return 0
        ...
        logging.error(f"Error getting total categories count: {e}")
        return 0
    finally:
        if conn and conn.is_connected(): cursor.close(); conn.close()

def get_low_stock_items_count(threshold=5):
    """Mengembalikan jumlah barang yang stoknya di bawah atau sama dengan threshold."""
    conn = None
    try:
        conn = create_connection()
        if not (conn and conn.is_connected()): return 0
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(item_id) FROM items WHERE stock <= %s", (threshold,))
        count = cursor.fetchone()[0]
        return count if count is not None else 0
    except Error as e:
        logging.error(f"Error getting low stock items count: {e}")
        return 0
    finally:
        if conn and conn.is_connected(): cursor.close(); conn.close()
